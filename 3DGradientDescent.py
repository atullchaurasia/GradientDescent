...
def calculate_gradient(x, y):
    return np.cos(5 * x) * np.cos(5 * 7), -np.sin(5 * x) * np.sin(5 * y)
...
current_position2 = (0.2, 0.8, z_function(0.7, 0.4))
current_position3 = (-0.5, 0.5, z_function(0.7, 0.4))
...
for _ in range(1000):
    ...
    ax.scatter(current_position2[0], current_position2[1], current_position2[2], color = "red", zorder=1)
    ax.scatter(current_position3[0], current_position3[1], current_position3[2], color = "yellow", zorder=1)
    plt.pause(0.001)
    ax.clear()